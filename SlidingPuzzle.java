...
public class SlidingPuzzle extends JFrame implements ActionListener
{
	...
	SlidingPuzzle() {
		for(int i=0; i < (row * col); i++) {
			img[i] = new ImageIcon(imageList[i]);
			shuffler.add(img[i]);
		...
		panel.setBounds(20, 40, 200 * col, 200 * row);
		panel.setLayout(new GridLayout(row, col, 0, 0));
		add(panel);

		shflBtn.setBounds(20, 10, (200 * col) / 2, 20);
		shflBtn.addActionListener(this);
		shflBtn.setFocusPainted(false);
		shflBtn.setContentAreaFilled(false);
		add(shflBtn);

		time.setBounds(((200 * col) / 2) + ((200 * col) / 2) / 2, 10, 200 * col, 20);
		add(time);

		credits.setBounds(((200 * col) / 2) - 65, (200 * row) + 45, 200 * col, 20);
		add(credits);
		...
		setSize((200 * col) + 55, (200 * row) + 115);
		setVisible(true);
